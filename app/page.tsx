...
export default function BirthdayWall() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cream via-peach-light to-pink-light relative overflow-hidden">
      ...
      <CashewDecoration className="absolute bottom-20 left-1/4 opacity-10" size="small" />
      <CashewDecoration className="absolute bottom-40 right-10 opacity-20" size="medium" />
      ...
      <div className="container mx-auto px-4 py-12 relative z-10">
        ...
        <header className="text-center mb-16 animate-fadeIn">
          ...
          <p className="text-xl text-primary-dark max-w-2xl mx-auto text-pretty leading-relaxed">
            Celebrando as mulheres fortes, criativas e apaixonadas por tecnologia
          </p>
        </header>
        ...
        {todaysBirthday && (
          <section className="mb-20 animate-slideUp">
            ...
            <TodaysBirthday person={todaysBirthday} />
          </section>
        )}
          ...
          {monthBirthdays.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {monthBirthdays.map((person, index) => (
                <div key={person.id} className="animate-fadeIn" style={{ animationDelay: `${300 + index * 100}ms` }}>
            ...
            <Card className="max-w-md mx-auto p-8 text-center bg-white/80 backdrop-blur-sm border-primary/20">
              <p className="text-muted-foreground text-lg">Nenhum aniversário este mês 🎈</p>
            </Card>
          )}
        </section>

        {/* Footer Message */}
        <footer className="mt-20 text-center animate-fadeIn" style={{ animationDelay: "600ms" }}>
          <div className="max-w-3xl mx-auto p-8 bg-white/60 backdrop-blur-sm rounded-3xl border border-primary/10">
            <p className="text-lg text-primary-dark leading-relaxed text-pretty italic">
              "As Cajuínas são mulheres que acolhem, incentivam, empoderam e capacitam. Unidos pela tecnologia,
              fortalecidas pela sororidade, orgulhosas de nossas raízes nordestinas."
            </p>
          </div>
        </footer>
      </div>
